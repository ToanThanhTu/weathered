...
- **Normalize at the boundary.** Convert upstream quirks here, not in the frontend. Example: `weather.service.ts` uses `localToUtcIso` to turn Open-Meteo's naive local time + `utc_offset_seconds` into a real UTC ISO string, and passes the IANA `timezone` through so the frontend can render in the city's local time via `Intl.DateTimeFormat({ timeZone })`. The service layer owns the "the API said X but the contract says Y" logic.
